...
def identity_block(X, f, filters, stage, block):
    ...
    X = Conv2D(filters=F3,padding='valid',strides=(1,1),kernel_size=(1,1),name=conv_name_base + '2c',kernel_initializer=glorot_uniform(seed=0))(X)
...
def convolutional_block(X, f, filters, stage, block, s = 2):
    ...
    filters -- python list of integers, defining the number of filters in the CONV layers of the main path
    stage -- integer, used to name the layers, depending on their position in the network
    block -- string/charact
    er, used to name the layers, depending on their position in the network
